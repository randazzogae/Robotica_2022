...
public class Robot_Left extends ControlSystemMFN150Explore{
    ...
    private NodeBooleanDinamic	PF_EXPLORE_CELLE, PF_RETURN_EXPLORE; //DEFINITI DA NOI PER GESTIRE AL MEGLIO LA FASE DI EXPLORE
  ...
  //      NodeVec2
    ...
    //    NodeInt
        ...
        //CREATI PER GESTIRE L'EXPLORE DELLE CELLE NON VISITATE
        PF_EXPLORE_CELLE=new NodeBooleanDinamic(false);
        PF_RETURN_EXPLORE=new NodeBooleanDinamic(false);
...
*/
       ...
       bring_plan.addAction("open_gripper",
        ...
        STATE_MACHINE.triggers[4][7]  =  PF_EXPLORE_CELLE;
        ...
        // Stato 10 - EXPLORE CELLE NON VISITATE:

        STATE_MACHINE.triggers[10][0]  =  PF_SOMETHING_VISIBLE; //ESCO SE C'E' QUALCOSA DI VISIBILE

        STATE_MACHINE.follow_on[10][0] = 1; 


         STATE_MACHINE.triggers[10][1]  =  PF_RETURN_EXPLORE;

        STATE_MACHINE.follow_on[10][1] = 4;
        ...
        /*
        STATE_MACHINE.triggers[10][5]  =  PF_NOT_ANTIROOM; //SE SEI ALLO STATO 4 E SCATTA TALE TRIGGER VAI NELLO STATO (DSIR) 9

        STATE_MACHINE.follow_on[10][5] = 9;


        STATE_MACHINE.triggers[10][6]  =  PF_NOT_ROOM;

        STATE_MACHINE.follow_on[10][6] = 1;
        */
        ...
        STEERING.embedded[10] = AS_GO_CHOOSE_ZONE;
        ...
        TURRET.embedded[10] = AS_GO_CHOOSE_ZONE;
        ...
        SONAR_CONFIGURATION.sonarActived[10] = new int[]{0,4,8,12};
        ...
        LASER_CONFIGURATION.embedded[10] = false;
    ...
    /*
        if(roomLocation.in_room_centre())
                {
                    //PF_EXPLORE_CELLE.setValue(true);
                    MOSTRA_REPORT.setReport("Il robot con id " + id + "è nella stanza centrale");
                    //return(CSSTAT_OK);
                }
                else
                {
                    MOSTRA_REPORT.setReport("***************************Il robot con id " + id + "NON è nella stanza centrale");
                }
                */

        if(state==10)
        {
            PF_RETURN_EXPLORE.setValue(false);
            PF_EXPLORE_CELLE.setValue(false);
            abstract_robot.setDisplayString("STATO 10 - EXPLORE CELLE NON VISITATE");
             double punto[] = new double[2];
            punto=localizer.isClosedZona1Point();
            PS_ATTRACTOR.setValue(curr_time,punto[0],punto[1]);
             if(roomLocation.in_blue_Left()||roomLocation.in_red_Left()||roomLocation.in_green_Left()||roomLocation.in_blue_Right()||roomLocation.in_green_Right()||roomLocation.in_red_Right())
                {
                     PS_ATTRACTOR.setValue(curr_time,punto[0],punto[1]);
                    PF_RETURN_EXPLORE.setValue(true); //vai allo stato 4
                }
        ...
        if(state==4){
           // if(!roomLocation.getAreClosedAllAntiRoom())
            //{
                ...
                if(roomLocation.in_room_centre())
                {
                    PF_EXPLORE_CELLE.setValue(true); //vado allo stato 10
                    /*swirlgain.setValue(0.5);
                    avoidgain.setValue(0.0);
                    noisegain.setValue(0.0);
                    PS_ATTRACTOR.setValue(curr_time, 6, 14.0);*/
                   // MOSTRA_REPORT.setReport("Il robot con id " + id + "è nella stanza centrale");
                    //return(CSSTAT_OK);
                }
