...
public class Robot_Left extends ControlSystemMFN150Explore{
    ...
    private NodeBooleanDinamic PF_GOTOSTATO10;
  ...
  //      NodeVec2
    ...
    //    NodeInt
        ...
        PF_GOTOSTATO10=new NodeBooleanDinamic(false);
...
*/
       ...
       bring_plan.addAction("open_gripper",
        ...
        STATE_MACHINE.triggers[7][3]=PF_GOTOSTATO10;
        STATE_MACHINE.follow_on[7][3]=10;   
        ...
        STATE_MACHINE.follow_on[10][3] = 7;
            	...
            	if (action.startsWith("goto(")){
                	...
                	if(bring_plan.getParameter()[0].equals("red_attractor2")){
                    	...
                    	if (PF_OTHER_ROBOT.Value(curr_time)&& (idFlag_other==PS_IN_GRIPPER.intValue(curr_time)))
                    		PS_ATTRACTOR.setValue(curr_time, 8.2, 24.3);
                		else
                			PS_ATTRACTOR.setValue(curr_time, 8.2, 24.3);
        ...
        if(state==4){ //STATO DI EXPLORE
            ...
            if(roomLocation.in_room_centre() && localizer.areClosedAllZones()) //SE SONO NELLA STANZA CENTRALE E LE ZONE STATE TUTTE VISITATE
            {
                if (localizer.amInZona1() || localizer.amInCorridoioD())
                {
                    ...
                    } else if (roomLocation.getIsClosedAntiRoomRedRight() == false) {
                    ...
                    else if (roomLocation.getIsClosedAntiRoomBlueRight() == false) {
...
*/
        ...
        if(state==7){
            ...
            if(roomLocation.getTempoVisitaLabirinto()<RoomLocation.TEMPO_MASSIMO_VISITA_LABIRINTO) //se si verifica cio', può fare la visita del labirinto
            {
                antistallo.resetI();
            ...
            else
            {
                ...
                punto[2]=12.0;
                punto[3]=10.0;
                //PS_ATTRACTOR.setValue(curr_time,10.5,14.5); //lo allontano prima e poi lo mando allo stato 10
                ...
                abstract_robot.setDisplayString("VOLEVO ENTRARE NEL LAB DALL'ALTO :'(");
                  swirlgain.setValue(0.5);
                    avoidgain.setValue(0.0);
                    noisegain.setValue(0.0);
                PF_GOTOSTATO10.setValue(true);
                return(CSSTAT_OK);
                //Setto il percorso per farlo allontanare dal labirinto (ricorda che punto[0] e punto[1] sono il punto attrattore finale e punto[2] e punto[3] è il punto intermedio)
                /*punto[0]=
                punto[1]=
                punto[2]=
                punto[3]=
            ...
            if(roomLocation.closed_lab_Right()){
                ...
                punto[2]=27.0;
                punto[3]=9.0;
                //PS_ATTRACTOR.setValue(curr_time,33.5,10.0); //lo allontano prima e poi lo mando allo stato 10
        ...
        if(state==10) //vai nella prima cella non libera di una zona o segui un determinato percorso composto da due celle
        {
            ...
            if(roomLocation.in_room() && localizer.areClosedAllZones()) //SE SEI ALLO STATO 10 IN UNA DELLE ANTICAMERE, SEI IN STALLO E QUINDI ESCI
            {
            ...
            if(antistallo.getI()>=400)
                {
                    System.out.println("ATTIVO ANTISTALLO DSIR");
                    PS_ATTRACTOR.setValue(curr_time,x_robot-0.2,y_robot+6);
                }
