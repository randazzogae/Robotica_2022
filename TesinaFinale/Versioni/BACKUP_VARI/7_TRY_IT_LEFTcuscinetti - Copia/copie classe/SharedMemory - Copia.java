...
public class SharedMemory {
    ...
    private ArrayList<Vec2> last_val=new ArrayList<Vec2>();
    private ArrayList<Integer> visual_number=new ArrayList<Integer>();

    public Vec2[] getLast_val() {
        return (Vec2[])last_val.toArray();
    }
    ...
    public Vec2[] value(MultiForageN150ExploreSim multi_robot,long timestamp, Vec2 position, Vec2[] im0_1, Vec2[] im0_2, Vec2[] im0_3) {
        ...
        if(im0.length !=0)
        {
            ...
            for(int i=k; i<last_val.size(); i++){
                last_val.set(i, new Vec2(im2[i - k].x, im2[i - k].y));
                visual_number.set(i, visual_old[i - k]);
            }
        }
