...
public class Robot_Left extends ControlSystemMFN150Explore{
    ...
    private NodeBooleanDinamic PF_CAN_VISIT_THAT_ANTIROOM; //Utile nel piano bring flag per vedere se, dopo la consegna della bandierina, il robot può visitare l'anticamera 
    ...
    private int anticameraDaVisitare; //serve per far visitare una certa anticamera dopo aver consegnato una bandierina.
        /*
            anticameraDaVisitare:
                                    =0 //blue di sinistra
                                    =1 //rossa di sinistra
                                    =2 //verde di sinistra
        */
  ...
  //      NodeVec2
    ...
    //    NodeInt
        ...
        PF_CAN_VISIT_THAT_ANTIROOM=new NodeBooleanDinamic(false);
...
*/
       ...
       bring_plan.addAction("open_gripper",
        ...
        bring_plan.addAction("reset_timer",
          ...
          bring_plan.addAction("valuta_anticamera",
                        NodeAction.NO_STEERING,
                        NodeAction.NO_TURRET,
                        NodeAction.IMMEDIATE_TRIGGER,
                        NodeAction.NO_GRIPPER,
        ...
        bring_plan.addAction("visit_antiroom", 
                       AS_GOTO, 
                       AS_GOTO,
                       PF_CLOSE_TO_OR_IN_LAB, 
                       0, 
        ...
        bring_plan.addAction("can_visit_that_antiroom", PF_CAN_VISIT_THAT_ANTIROOM); //Aggiunto per verificare se, dopo aver consegnato la bandierina, può visitare quell'anticamera all'uscita
		...
		STATE_MACHINE.triggers[2][1]  = PF_NOT_ANTIROOM;
        STATE_MACHINE.follow_on[2][1] = 9;
       ...
       //ANTISTALLO DSIR - incremento la variabile i ad ogni takestep()
        ...
        if(state==0){
         ...
         if(state==1){
            ...
            PF_CAN_VISIT_THAT_ANTIROOM.setValue(false);
    ...
    if(state==2){
        ...
        if(bring_plan.isChangedState()){
            ...
            if(action.startsWith("valuta_anticamera")) //aggiunta questa azione per valutare se, dopo aver consegnato la bandierina, il robot può visitare quell'anticamera
            {
                antistallo.resetI();
                abstract_robot.setDisplayString("VALUTA ANTICAMERA");
                if((roomLocation.closed_blue_Left() || roomLocation.in_blue_Left()) && roomLocation.getTempoVisitaAnticameraBlueLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA && flag_memory.getLength(curr_time)==0)
                {
                    anticameraDaVisitare=0; //blue di sinistra
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                    System.out.println("********IL ROBOT " + id + " PUO' VISITARE QUELL'ANTICAMERA ------ anticamera da visitare: " +anticameraDaVisitare);
                }
                else if ((roomLocation.closed_red_Left() || roomLocation.in_red_Left()) && roomLocation.getTempoVisitaAnticameraRedLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA && flag_memory.getLength(curr_time)==0)
                {
                    anticameraDaVisitare=1; //rossa di sinistra
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                    System.out.println("********IL ROBOT " + id + " PUO' VISITARE QUELL'ANTICAMERA ------ anticamera da visitare: " +anticameraDaVisitare);
                }
                else if ((roomLocation.closed_green_Left() || roomLocation.in_green_Left()) && roomLocation.getTempoVisitaAnticameraGreenLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA && flag_memory.getLength(curr_time)==0)
                {
                    anticameraDaVisitare=2; //verde di sinistra
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                    System.out.println("********IL ROBOT " + id + " PUO' VISITARE QUELL'ANTICAMERA ------ anticamera da visitare: " +anticameraDaVisitare);
                }
                else
                {
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(false);
                    System.out.println("********IL ROBOT " + id + " NOOOOOOOOOOOON VISITARE QUELL'ANTICAMERA");
                }
              
            } //fine valuta anticamera
            else
            if(action.startsWith("visit_antiroom")) //aggiunta questa azione per valutare se, dopo aver consegnato la bandierina, il robot può visitare quell'anticamera
            {
                abstract_robot.setDisplayString("AZIONE VISITO ANTICAMERA");
                antistallo.resetI();
                if(anticameraDaVisitare==0) //blue di sinistra
                {
                    System.out.println("***********VISITO L'ANTICAMERA BLUE DI SINISTRA");
                ...
                else if(anticameraDaVisitare==1) //rossa di sinistra
                {
                    System.out.println("***********VISITO L'ANTICAMERA ROSSA DI SINISTRA");
                ...
                else if(anticameraDaVisitare==2) //verde di sinistra
                {
                    System.out.println("***********VISITO L'ANTICAMERA VERDE DI SINISTRA");
                ...
                /*if((roomLocation.closed_blue_Left() || roomLocation.in_blue_Left()) && roomLocation.getTempoVisitaAnticameraBlueLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA )
                {
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                }
                else if ((roomLocation.closed_red_Left() || roomLocation.in_red_Left()) && roomLocation.getTempoVisitaAnticameraRedLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA)
                {
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                }
                else if ((roomLocation.closed_green_Left() || roomLocation.in_green_Left()) && roomLocation.getTempoVisitaAnticameraGreenLeft()<roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA)
                {
                    PF_CAN_VISIT_THAT_ANTIROOM.setValue(true);
                }
                else
        ...
        if(state==4){ //STATO DI EXPLORE
           // if(!roomLocation.getAreClosedAllAntiRoom())
            //{
             ...
             /*if(roomLocation.closed_lab_())
            {
                antistallo.resetI();
                PF_ENTRY.setValue(true); //vado allo stato 7
             ...
             if(roomLocation.in_room_centre() && !localizer.areClosedAllZones()) //SE SONO NELLA STANZA CENTRALE E LE ZONE NON SONO ANCORA TUTTE VISITATE
                {
                   
                    if(localizer.amInZona1() || localizer.amInCorridoioD()) 
                    {
                        if(!localizer.isClosedZona1()) //NOTA: riuscire a far restiture direttamente un booleano al secondo metodo (anzichè fare il for)
                        {
                    ...
                    {
                        if(!localizer.isClosedZona2()) //NOTA: riuscire a far restiture direttamente un booleano al secondo metodo (anzichè fare il for)
                        {
                    ...
                    {
                        if(!localizer.isClosedZona4()) //NOTA: riuscire a far restiture direttamente un booleano al secondo metodo (anzichè fare il for)
                        {
                    ...
                    {
                        if(!localizer.isClosedZona3()) //NOTA: riuscire a far restiture direttamente un booleano al secondo metodo (anzichè fare il for)
                        {
                    ...
                    /*if(!localizer.isClosedZona1()) //se non è chiusa la zona1 (cambiare con un booleano nella classe location)
                    {
                        punto=localizer.isClosedZona1Point();
                        antistallo.resetI();
                        PF_EXPLORE_CELLA.setValue(true); //vai allo stato 10
                    }
                    else //Se la zona 1 è chiusa faccio qualcosa per non farlo bloccare
                    {
                        PF_ENTRY.setValue(true); //vai allo stato7 (SI AVVICINA AL LABIRINTO)
                    }
                    */
                    /*if(roomLocation.closed_lab_())
                    {
      ...
      if(state==7){
        ...
        if(state==10) //vai nella prima cella non libera di una zona o segui un determinato percorso composto da due celle
        {
            ...
            if(antistallo.getI()>=400)
                {
                    System.out.println("ATTIVO ANTISTALLO DSIR");
                    PS_ATTRACTOR.setValue(curr_time,x_robot-1,y_robot-1);
                }
