...
primitive_action(visit_antiroom).
primitive_action(valuta_anticamera).
...
poss(open_gripper,_).
poss(reset_timer,_).
poss(valuta_anticamera,_).
poss(visit_antiroom,_).
poss(go_flag,_).
poss(exit,_).
...
holds(can_go,do(A,S)):-  (holds(red_flag,S),   A = goto(red_attractor)  ); 
			 ...
			 A=assm(can_go,1).
holds(can_visit_that_antiroom,do(A,S)):- holds(can_visit_that_antiroom,S) ; A = assm(can_visit_that_antiroom,1).
...
proc(go_bin(Z,X,Y),
        ...
        (sense(in_lab):
         branch_on(in_lab):
	       ...
	       goto(X) : send_message(am_at_attractor) : valuta_anticamera:
	       (sense(can_go):
	 branch_on(can_go):
	 if(can_go,
		...
		reset_timer:
		(sense(can_visit_that_antiroom):
			branch_on(can_visit_that_antiroom):
				if(can_visit_that_antiroom,
					visit_antiroom: goto(Z),
					goto(Z)))
).