...
holds(something_visible_attack,do(A,S)):- holds(something_visible_attack,S) ; A = assm(something_visible_attack,1).
...
proc(attack,
		goto(prima_cella):
		(sense(something_visible_attack):
		branch_on(something_visible_attack):
		if(something_visible_attack,
		   open_gripper:go_flag,
			goto(seconda_cella))):
		goto(seconda_cella):
		(sense(something_visible_attack):
		branch_on(something_visible_attack):
...
proc(attack,
		...
		if(something_visible_attack,
			...
			branch_on(have_flag):
			if(have_flag,
				open_gripper,go_flag,goto(seconda_cella),goto(seconda_cella)   
			)
			),
			goto(seconda_cella)))
).
		...
		*/
		...
		if(something_visible_attack,
		   (sense(have_flag):
			branch_on(have_flag):
			if(have_flag,
				open_gripper:go_flag:goto(terza_cella),goto(terza_cella)   
			)
			):
			)):
			(sense(something_visible_attack):
		...
		if(something_visible_attack,
		   (sense(have_flag):
			branch_on(have_flag):
			if(have_flag,
				open_gripper:go_flag:goto(quarta_cella),goto(quarta_cella)   
			)
			):
			)):
			(sense(something_visible_attack):
		...
		if(something_visible_attack,
		   (sense(have_flag):
			branch_on(have_flag):
			if(have_flag,
				open_gripper:go_flag:goto(quinta_cella),goto(quinta_cella)   
			)
			):
			))
