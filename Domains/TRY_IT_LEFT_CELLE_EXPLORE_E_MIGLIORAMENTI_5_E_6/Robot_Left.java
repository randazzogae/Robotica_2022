...
public class Robot_Left extends ControlSystemMFN150Explore{
	...
	private NodeVec2            PS_CLOSEST_FLAG_FOR_ATTACK;
    ...
    private NodeBooleanDinamic  PF_OTHER_ROBOT,PF_STALL,PF_ROOM,PF_ENTRY,PF_EXIT_CENTRE,PF_CENTRE, PF_EXIT_STALL; //HO CREATO PF_EXIT_STALL PER GESTIRE SITAZIONI DI STALLO
     private NodeBooleanDinamic PF_NOT_ANTIROOM; //HO CREATO QUESTO PER GESTIRE LE SITUAZIONI IN CUI UN ROBOT NON PUO' VISITARE PIU' UNA DETERMINATA ANTICAMERA
	private NodeBooleanDinamic PF_BRING_FLAG; //HO CREATO QUESTO TRIGGER PER USCIRE DALLA FASE DI ATTACK E ANDARE NEL PIANO BRING_FLAG SE IL ROBOT HA UNA BANDIERINA
    ...
    private SGologPlan          bring_plan,lab_plan,attack_plan;    
    private NodeMap             MAP;
    private NodeMemory_Try_it   flag_memory;
	private MemoriaSingleton memoria_condivisa;
	...
	private Vec2 attack_flag=new Vec2(0,0);
    ...
    switch(id){
		...
		attack_plan = new SGologPlan(abstract_robot); 
        
        MAP = new NodeMap(0.0, 44.0,30.0, 0.0,0.8,0.7,"Mappa",false,abstract_robot);
        
        
	    flag_memory=new NodeMemory_Try_it(true,2,1,0,ab); //variabile d'istanza control=true, ab = abstract robot e le tre classi visuali delle bandierine
	    memoria_condivisa=MemoriaSingleton.getInstance();
		System.out.println("-----FLAG MEMORY-----");
		System.out.println(flag_memory);

		System.out.println("-----MemoriaSingleton-----");
		System.out.println(memoria_condivisa);
    ...
    //   NodeVec2
		...
		NodeVec2Array
        PS_ALL_TARGET_FOR_ATTACK = new va_Merge_vava(PS_TEMP, PS_BLUE_FLAG_EGO);
        
  //      NodeVec2
        PS_CLOSEST_FLAG = new v_Closest_va(PS_ALL_TARGET);
        
		//PS_CLOSEST_FLAG_FOR_ATTACK = new v_Closest_va_dsir(PS_ALL_TARGET_FOR_ATTACK);
    ...
    //    NodeInt
		...
		NodeBoolean
        PF_SOMETHING_VISIBLE_AND_NOT_IN_GRIPPER=new b_NonZero_v(PS_CLOSEST_FLAG);

		//NodeBoolean
        //PF_SOMETHING_VISIBLE_FOR_ATTACK=new b_NonZero_v(PS_CLOSEST_FLAG_FOR_ATTACK);
		...
		//CREO UN NUOVO TRIGGER PER GESTIRE LE SITUAZIONI IN CUI UN ROBOT DEVE ANDARE DALLA FASE DI ATTACK A QUELLA DI BRING_FLAG
        PF_BRING_FLAG=new NodeBooleanDinamic(false);
...
*/
        ...
        *************/
		...
		attack_plan.debugging(false);
		
		attack_plan.addAction("goto", 
                       AS_GOTO, 
                       AS_GOTO,
		...
		attack_plan.addAction("open_gripper",
		...
		attack_plan.addAction("something_visible_attack", PF_SOMETHING_VISIBLE);
		attack_plan.addAction("have_flag", PF_HAVE_FLAG);
		attack_plan.setPlan(client.invoke("do(attack,s0,S)"),0);
       ...
       bring_plan.addAction("open_gripper",
        ...
        STATE_MACHINE.triggers[0][0]  = attack_plan.isFinished(abstract_robot.getTime());     //Stato 0:ATTACK
        STATE_MACHINE.follow_on[0][0] = 2; 

		/*STATE_MACHINE.triggers[0][0]  = PF_SOMETHING_IN_GRIPPER;     //Stato 0:ATTACK
        STATE_MACHINE.follow_on[0][0] = 2;*/
		
		/*STATE_MACHINE.triggers[0][0]  = PF_BRING_FLAG;     //Stato 0:ATTACK
        STATE_MACHINE.follow_on[0][0] = 2; */
		

		//STATE_MACHINE.triggers[0][1]  = PF_SOMETHING_VISIBLE;     //Stato 0:ATTACK
        //STATE_MACHINE.follow_on[0][1] = 2;      //Se sei allo stato 0 e hai qualcosa nel gripper vai allo stato 2 (bring_plan)
		...
		STATE_MACHINE.triggers[4][4]  =  PF_NOT_ANTIROOM; //SE SEI ALLO STATO 4 E SCATTA TALE TRIGGER VAI NELLO STATO (DSIR) 9
		STATE_MACHINE.follow_on[4][4] = 9;
        ...
        STEERING.embedded[0] = attack_plan.getSteering(abstract_robot.getTime());
        ...
        TURRET.embedded[0] = attack_plan.getTurret(abstract_robot.getTime());;
        ...
        GRIPPER_FINGERS.embedded[0] = attack_plan.getGripperFingers(abstract_robot.getTime());
        ...
        SONAR_CONFIGURATION.sonarActived[0] = attack_plan.getSonar(abstract_robot.getTime());
        ...
        LASER_CONFIGURATION.embedded[0] = attack_plan.getLaser(abstract_robot.getTime());;
    ...
    public int takeStep(){
		...
		if(bufferMessage.hasMoreElements()) 
        {
            ...
            try
            {
                ...
                if(idMessage.val == other_id && bufferMessage.hasMoreElements()) 
                {
                    ...
                    {
                    	PointMessage flagPosition=(PointMessage) bufferMessage.nextElement(); //Ottengo un oggetto PointMessage (avente attributi x e y) leggendo dal buffer la bandierina nuova
                    	Vec2 pos=new Vec2 (flagPosition.xValue,flagPosition.yValue); //Creo un oggetto vec 2 con le coordinate della bandierina appena recuperata dal buffer		
                    	LongMessage visualMessage =(LongMessage) bufferMessage.nextElement(); //recupero la classe visuale della bandierina
                    	int vis=(int)visualMessage.val;
                    	flag_memory.push(pos,vis,curr_time); //inserisco in memoria la posizione della bandierina e la classe visuale
						MemoriaSingleton.getInstance().addFlag(pos);
						MemoriaSingleton.getInstance().showList();
					}
                    else if(message.val=="Delete Flag")
                    {
                    	PointMessage flagPosition=(PointMessage) bufferMessage.nextElement(); //Ottengo un oggetto PointMessage (avente attributi x e y) leggendo dal buffer la bandierina nuova
                    	Vec2 pos=new Vec2 (flagPosition.xValue,flagPosition.yValue); //Creo un oggetto vec 2 con le coordinate della bandierina appena recuperata dal buffer	
                    	flag_memory.pop_only(pos,curr_time);  //rimuovo dalla memoria la bandierina avente posizione pos	
                    	
						//MemoriaSingleton.getInstance().removeFlag(pos);
						//MemoriaSingleton.getInstance().getList().remove(pos);
						MemoriaSingleton.getInstance().showList();
					}
                    else  
		...
		//ANTISTALLO DSIR - IN OGNI STATO VERRA' FATTA UNA VERIFICA DEL VALORE DI I, FACENDO SCATTARE UN 
		//OPPORTUNO TRIGGER
...
if(roomLocation.in_red_Right()){      	
        ...
        if(state==0){
        	/*
			/* --COMMENTANDO IL SEGUENTE CODICE ENTRAMBI I ROBOT SI BLOCCANO */
			/*if(y_robot<15.0)
			...
			*/ 
			...
			if(id==1)
			{
				//percorso iniziale robot 1
				PS_ATTRACTOR.setValue(curr_time,32.0,7.0); //cella 54 
			}
			else if(id==0)
			{ */
				
				/*
				//percorso iniziale robot 0
				if(localizer.amInCella8(x_robot,y_robot)||localizer.amInCella16(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,26.0,42.0); //vai alla cella 7
					//linear_attraction2.setRot_teta(0.0);
					System.out.println("**************************************************Sto andando nella prima cella");
				}
				else if(localizer.amInCella7(x_robot,y_robot)||localizer.amInCella15(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,24.0,25.0); //vai alla Cella 6
					System.out.println("**************************************************Sto andando nella seconda cella");
				}
				else if(localizer.amInCella6(x_robot,y_robot)||localizer.amInCella14(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,23.0,27.0); //vai alla cella 13
					System.out.println("**************************************************Sto andando nella terza cella");
				}
				else if(localizer.amInCella5(x_robot,y_robot)||localizer.amInCella13(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,20.0,27.0); //vai alla cella 4
					System.out.println("**************************************************Sto andando nella quarta cella");
				}
				else if(localizer.amInCella4(x_robot,y_robot)||localizer.amInCella12(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,17.0,27.0); //vai alla cella 3
					System.out.println("**************************************************Sto andando nella quinta cella");
				}
				else if(localizer.amInCella3(x_robot,y_robot)||localizer.amInCella11(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,14.0,27.0); //vai alla cella 2
					System.out.println("**************************************************Sto andando nella sesta cella");
				}
				else if(localizer.amInCella2(x_robot,y_robot)||localizer.amInCella10(x_robot,y_robot))
				{
					//PS_ATTRACTOR.setValue(curr_time,12.0,27.0); //vai alla cella 1
					System.out.println("**************************************************Sto andando nella settima cella");
				}
				*/
			...
			//-------------------------------
				attack_plan.update(curr_time, state);
				if(antistallo.getI()>=60)
					{
				abstract_robot.setDisplayString("ATTIVO ANTISTALLO DSIR");
				antistallo.resetI();
				PF_EXIT_STALL.setValue(true);
				...
				if(attack_plan.isChangedState()){
				action = attack_plan.getActionName();

				if(action.startsWith("open_gripper")){
					MOSTRA_REPORT.setReport("----------Il robot " + id + " ha rilevato una bandierina e sta aprendo il gripper\n");
				...
				if(action.startsWith("go_flag")){
 					...
 					begin_flag=false;
 					antistallo.update();
 					 //attack_flag=new Vec2(PS_CLOSEST_FLAG.Value(curr_time).x+x_robot,PS_CLOSEST_FLAG.Value(curr_time).y+y_robot);
        
 								
 					//flag_memory.pop(attack_flag,curr_time);
					//MemoriaSingleton.getInstance().removeFlag(attack_flag);
 					//PF_HAVE_FLAG.setValue(true); 
 					//COMMENTO PER ORA ----->PF_HAVE_FLAG_LAB.setValue(true);
				...
				if(action.startsWith("goto(")){
						if(attack_plan.getParameter()[0].equals("prima_cella")){
								if(id==0)
								{
									PS_ATTRACTOR.setValue(curr_time,29.0,27.0); //vai alla cella 7
								}
								else
								{
									PS_ATTRACTOR.setValue(curr_time,28.0,20.0);
								}
								abstract_robot.setDisplayString("VADO NELLA PRIMA CELLA - ATTACK");
								System.out.println("**********Sto eseguendo la prima azione del piano");
								antistallo.update();
						}
						else
						if(attack_plan.getParameter()[0].equals("seconda_cella")){
								if(id==0)
								{
									//PS_ATTRACTOR.setValue(curr_time,26.0,15.0); //vai alla cella 6 - valore di prova verso il basso
									//PS_ATTRACTOR.setValue(curr_time,26.0,27.0); //vai alla cella 6
									PS_ATTRACTOR.setValue(curr_time,26.0,25.0);
								}
								else
								{
									PS_ATTRACTOR.setValue(curr_time,27.0,15.0);
								}
						abstract_robot.setDisplayString("VADO NELLA SECONDA CELLA - ATTACK");
						System.out.println("**********Sto eseguendo la seconda azione del piano");
						antistallo.update();
						}
						else
						if(attack_plan.getParameter()[0].equals("terza_cella")){
								if(id==0)
								{
									PS_ATTRACTOR.setValue(curr_time,24.0,27.0); //vai alla cella 5
								}
								else
								{
									PS_ATTRACTOR.setValue(curr_time,28.0,16.0);
								}
						abstract_robot.setDisplayString("VADO NELLA TERZA CELLA - ATTACK");
						System.out.println("**********Sto eseguendo la terza azione del piano");
						//PF_BRING_FLAG.setValue(true);
						antistallo.update();
						}
						else
						if(attack_plan.getParameter()[0].equals("quarta_cella")){
							if(id==0)
								{
									PS_ATTRACTOR.setValue(curr_time,18.0,25.0); //vai alla cella 4
								}
								else
								{
									PS_ATTRACTOR.setValue(curr_time,39.0,13.0);
								}
						abstract_robot.setDisplayString("VADO NELLA QUARTA CELLA - ATTACK");
						System.out.println("**********Sto eseguendo la quarta azione del piano");
						antistallo.update();
						}
						else
						if(attack_plan.getParameter()[0].equals("quinta_cella")){
							if(id==0)
								{
									PS_ATTRACTOR.setValue(curr_time,16.0,27.0); //vai alla cella 3
								}
								else
								{
									PS_ATTRACTOR.setValue(curr_time,22.0,10.0);
								}
						abstract_robot.setDisplayString("VADO NELLA QUINTA CELLA - ATTACK");
						System.out.println("**********Sto eseguendo la quinta azione del piano");
						antistallo.update();
						}
				}
         ...
         if(state==1){
    		...
    		if(roomLocation.in_room()){
    			...
    			if(count_in_room>120){
					...
					MemoriaSingleton.getInstance().removeFlag(flag);
    ...
    if(state==2){
        ...
        if(PF_HAVE_FLAG_LAB.Value(curr_time)==true){
			...
			MemoriaSingleton.getInstance().removeFlag(lab_flag);
        ...
        //          Esce da questa stato perch� uno dei due robot ha avvistato qualcosa          
        if(state==4){
        	...
        	//if(roomLocation.in_room()){
				if(roomLocation.in_blue_Left())
				{
					if(roomLocation.getIsClosedAntiRoomBlueLeft()==true && roomLocation.getTempoVisitaAnticameraBlueLeft()>=roomLocation.TEMPO_MASSIMO_VISITA_ANTICAMERA)
					{
						...
						//PS_ATTRACTOR.setValue(curr_time,16.0,15.0);
						//PF_ENTRY.setValue(true);
  ...
  public void trialEnd(){
	...
	if(MOSTRA_REPORT!=null) MOSTRA_REPORT.dispose();
