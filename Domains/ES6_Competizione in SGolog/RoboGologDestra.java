...
public class RoboGologDestra extends ControlSystemMFN150Explore{
    ...
    public void configure(){
       ...
       plan.addAction("go_flag", 
        ...
        plan.addAction("wait",
                        AS_GOTO,
                        AS_GOTO,
                        PF_OTHER_ROBOT, //indica la fine dell'azione wait (se true)
                        NodeAction.NO_GRIPPER,
                        NodeAction.NO_SONAR,
                        NodeAction.NO_LASER);
        ...
        plan.addAction("can_go", PF_OTHER_ROBOT); //IL TRIGGER OTHER ROBOT E' ASSOCIATO AL SENSE CAN_GO
    ...
    public int takeStep(){
        ...
        if(plan.isChangedState()){
            ...
            {
                System.out.println("ROBO ROSSO-wait");
            }
            else
            if(action.startsWith("send_message(am_at_attractor")){
                //System.out.println("Destra invia messaggio a robot" + other_id);
                try
                {                        
                    LongMessage idMessage = new LongMessage();
                    idMessage.val = abstract_robot.getPlayerNumber(curr_time); //id del robot che sta inviando il messaggio
                    abstract_robot.unicast(other_id, idMessage); //Tramite other_id (id dell'altro robot), manda il messaggio in modalità UNICAST all'altro robot.
                    StringMessage message = new StringMessage();
                    message.val = "Pronto";
                    abstract_robot.unicast(other_id, message); //un robot invia il messaggio message all'altro robot avente id other_id
                }
            catch(CommunicationException e){}  
            }
            else if(action.startsWith("wander")) PF_OTHER_ROBOT.setValue(false); //SE SONO IN WANDER, NON POSSO ANDARE
        }
        ...
        if(bufferMessage.hasMoreElements()) 
        {
            try
            {
                //prendo il primo messaggio dal buffer, che sara' l'id del mittente:
                LongMessage idMessage = (LongMessage) bufferMessage.nextElement();

                //verifico tale id e verifico nuovamente se nel buffer ci sono altri messaggi:  
                if(idMessage.val == other_id && bufferMessage.hasMoreElements()) 
                {
                    ...
                    {
                    //scatta il trigger PF_OTHER_ROBOT, che indica la fine dell'azione wait e can_go adesso sara' vero.
