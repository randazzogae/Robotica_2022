...
public class Agent extends ControlSystemMFN150
	...
	private i_FSA_ba                STATE_MACHINE;
        ...
        private int                     time = 0;
        private NodeBooleanDinamic      PF_MESSAGE_RECEIVED; //associata al sense message_received e utile a far terminare l'azione wait.
        private NodeBooleanDinamic      PF_WAIT_IS_OVER; //trigger per indicare la fine dell'attesa (se true, attesa finita)

        private int i=0; //utile per l'attesa
        private int aiuto=0; //utile per l'attesa, per la conversione in secondi
        ...
        //private boolean aspetta=false; NON UNA BUONA SOLUZIONE!!!
	...
	public void configure(){
            ...
            abstract_robot.setBaseSpeed(0.9*abstract_robot.MAX_TRANSLATION);
            ...
            PF_MESSAGE_RECEIVED = new NodeBooleanDinamic(false);
            PF_WAIT_IS_OVER = new NodeBooleanDinamic(false);
           ...
           plan.addAction("board_plane", 
              ...
              plan.addAction("stop", 
                       AS_GOTO, 
                       AS_GOTO,
                       PF_MESSAGE_RECEIVED, //tale trigger sara' false se il messaggio non viene ricevuto, quindi rimarrà in stop sempre 
                       NodeAction.NO_GRIPPER);
           ...
           plan.addAction("drop_flower", 
             ...
             plan.addAction("wait", 
                   AS_GOTO, 
                   AS_GOTO,
                   PF_WAIT_IS_OVER, //SE SCATTA IL TRIGGER, l'azione wait termina.
                   NodeAction.NO_GRIPPER);
           ...
           plan.addAction("message_received", PF_MESSAGE_RECEIVED); //associo il trigger al sense
       ...
       //.......
            ...
            if(plan.isChangedState()){
                ...
                if (action.startsWith("drop_flower")){
                    ...
                    //PS_ATTRACTOR.setValue(curr_time, 1.30, 20.70); //PUNTO DI ATTESA

                    
                } else
                if (action.startsWith("wait")){
                    //aspetta=true;  
                    PS_ATTRACTOR.setValue(curr_time, 12.0, 0.9); 
                    //PS_ATTRACTOR.setValue(curr_time, 11.8, 6.2); //punto attrattore vicino imbarco c
                    attesa=true;
                    //PS_ATTRACTOR.setValue(curr_time, 3, 1.10); //PUNTO DI ATTESA
                    //abstract_robot.setSpeed(curr_time,0.0);
                    //AGENTE ESEGUIRA' SEMPRE L'AZIONE WAIT.
                    //QUANDO SCATTA IL TRIGGER PF_WAIT_IS_OVER, TALE AZIONE TERMINA.
				} else
                if(action.startsWith("stop"))
                {
                    PS_ATTRACTOR.setValue(curr_time, 15.0, 6.00);
                }
                else
             ...
             //Agent controlla se ci sono messaggi nel buffer:
        if (BufMsg.hasMoreElements())
          {
 			...
 			if (BufMsg.hasMoreElements()){
                ...
                attesa=false;
                PF_WAIT_IS_OVER.setValue(true); //scatta tale trigger che indica la fine dell'azione wait
                PF_MESSAGE_RECEIVED.setValue(true); //scatta tale trigger: message_received=true. 
                
                PS_ATTRACTOR.setValue(curr_time, 10.1, 0.7); //punto attrattore di piazza
                abstract_robot.setSpeed(curr_time,90.0); //NON SERVE setSpeed.
                return(CSSTAT_OK);
        ...
        public void attesa()
        {
        //Il robot agent aspetta (10 secondi == 1000). 
            
            
            if(i<400) //attesa dell'agent
            {
                i=i+1;
                if(aiuto==(int)(i/100))
				{
					System.out.println("AGENT wait: " +i/100+" sec");
				    aiuto++;
				}
				  
            }
            else if(i==400) //quando raggiunge il tempo massimo di attesa
            {
                PF_WAIT_IS_OVER.setValue(true);
                PF_MESSAGE_RECEIVED.setValue(false); //attesa finita, messaggio non ricevuto
                attesa=false;
            }
            else
            {
