...
primitive_action(stop). %aggiunta questa azione primitiva che lo fa rimanere fermo al bar se non riceve il messaggio
...
proc(girl, %---MODIFICATO  QUI 
	  ...
	  if(message_received,goto(piazza):goto(imbarco_C),goto(piazza):goto(imbarco_C):goto(bar):stop)	     
).
