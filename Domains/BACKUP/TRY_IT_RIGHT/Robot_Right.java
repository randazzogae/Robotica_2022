...
public class Robot_Right extends ControlSystemMFN150Explore{
    ...
    private NodeGain            noisegain = new NodeGain(0.2);
    ...
    private NodeBooleanDinamic  PF_IN_LOW_LAB,PF_STALL_FLAG,PF_IN_CENTRE_LAB;
    private NodeBooleanDinamic  PF_HAVE_FLAG,PF_HAVE_FLAG_LAB,PF_IN_LAB,PF_USE_LAB,PF_USE_LAB_RED_BLUE;
  ...
  //      NodeVec2
	    ...
	    NodeVec2 bin_red=new v_FixedPoint_(41.5, 23);
	    NodeVec2 bin_green=new v_FixedPoint_(41.5, 14);
	    NodeVec2 bin_blue=new v_FixedPoint_(41.5,5);
...
*/
    ...
    public int takeStep(){
        ...
        if(state==0){	
        	if(y_robot<8.0)
        		PS_ATTRACTOR.setValue(curr_time,22.0,7.0);
        	else
         ...
         if(state==1){
         	...
         	PF_HAVE_FLAG.setValue(false);
       		count_3=0;
       		delete=true;  // Utilizzata per eliminare la bandierina nello stato 5
    ...
    if(state==2){
        ...
        else
        {
      	...
      	if(roomLocation.use_lab_left()){
      		PF_USE_LAB.setValue(true);
      	}
      	else PF_USE_LAB.setValue(false);
      	
      	if(roomLocation.use_lab_left_redBlue()){
            	...
            	if (action.startsWith("goto(")){
                	...
                	if(bring_plan.getParameter()[0].equals("red_attractor2")){
                    	...
                    	if (PF_OTHER_ROBOT.Value(curr_time)&& (idFlag_other==PS_IN_GRIPPER.intValue(curr_time)))
                    		PS_ATTRACTOR.setValue(curr_time, 36.0, 25.0);
                		else
                			PS_ATTRACTOR.setValue(curr_time, 36.0, 24.4);
                	...
                	if(bring_plan.getParameter()[0].equals("green_attractor2")){
                    	...
                    	if (PF_OTHER_ROBOT.Value(curr_time)&& (idFlag_other==PS_IN_GRIPPER.intValue(curr_time)))
                   			PS_ATTRACTOR.setValue(curr_time, 38.0, 13.2);
                 		else
                			PS_ATTRACTOR.setValue(curr_time, 38.0, 14.8);
                	}
                	else
                	if(bring_plan.getParameter()[0].equals("blue_attractor2")){
                        ...
                        if (PF_OTHER_ROBOT.Value(curr_time)&& (idFlag_other==PS_IN_GRIPPER.intValue(curr_time)))
                    		PS_ATTRACTOR.setValue(curr_time, 36.0, 3.0);
                		else
                			PS_ATTRACTOR.setValue(curr_time, 36.0, 3.6);
                	}
                	else
                	if(bring_plan.getParameter()[0].equals("red_attractor1")){
                    	PS_ATTRACTOR.setValue(curr_time, 32.5, 23.0);
                	...
                	if(bring_plan.getParameter()[0].equals("green_attractor1")){
                    	PS_ATTRACTOR.setValue(curr_time, 31.6, 14);
                    	if(x_robot>11 && x_robot<19 && y_robot>=15 && y_robot<22){
                    		linear_attraction2.setRot_teta(1.2);
                    		//System.out.println("Sono nella stanza 8a-9");
                    	}
                    	else if(x_robot>11 && x_robot<19 && y_robot<15 && y_robot>=8){
                    		linear_attraction2.setRot_teta(-1.1);
                    		//System.out.println("Sono nella stanza 7-8b");
                    	}
                    	else if(x_robot>11 && x_robot<22 && y_robot>0 && y_robot<8){
                    		linear_attraction2.setRot_teta(-0.8);
                    		//System.out.println("Sono nella stanza 6");
                    	}
                    	else if(x_robot>=11 && x_robot<=22 && y_robot>=22 && y_robot<28){
                    		linear_attraction2.setRot_teta(0.5);
                    		//System.out.println("Sono nella stanza 10");
                    	}
                	}
                	else
                	if(bring_plan.getParameter()[0].equals("blue_attractor1")){
                    	PS_ATTRACTOR.setValue(curr_time, 32.5, 5.0);
                    	     if(x_robot>=17 && x_robot<25 && y_robot>=22&& y_robot<=25){         		
                    		linear_attraction2.setRot_teta(0.35);
                    		//System.out.println("Sono nella stanza 10a");                    		
                    	}
                    	else if(x_robot>=11 && x_robot<=17 && y_robot>=22&& y_robot<=27){         		
                    		linear_attraction2.setRot_teta(0.5);
                    		//System.out.println("Sono nella stanza 10b");                    		
                    	}                    
                    	else if(x_robot>=11 && x_robot<=19 && y_robot>8 && y_robot <=22){
                    		linear_attraction.setRot_teta(-0.7);
                    		//System.out.println("Sono nella stanza 7-8-9");
                    	}
                    	else{
                    		linear_attraction2.setRot_teta(0.0);
                    	}
                	...
                	if(bring_plan.getParameter()[0].equals("entry_lab")){
                    	PS_ATTRACTOR.setValue(curr_time, 18, 20);
        ...
        if(state==4){
    		...
    		if((!red_room_Right&&roomLocation.closed_red_Right())||(!green_room_Right&&roomLocation.closed_green_Right())||(!blue_room_Right&&roomLocation.closed_blue_Right())||(!lab_room&&roomLocation.closed_lab_())||(!red_room_Left&&roomLocation.closed_red_Left())||(!green_room_Left&&roomLocation.closed_green_Left())||(!blue_room_Left&&roomLocation.closed_blue_Left()))
    		{
        ...
        // Stato 5: E' uno stato intermedio tra la localizzazione e direzione immediata 
 		//          verso la bandiera avvistata (stato 1) e la pronta consegna (stato 2). 
       	if(state==5){
            begin1=true;
        	tempo=0;   //Variabile contatore utilizzata nello stato 2
        	flag=flag_closest.Value(curr_time);
        	PS_ATTRACTOR_FLAG.setValue(curr_time,flag.x,flag.y);
        	PS_ATTRACTOR.setValue(curr_time,flag.x,flag.y);
      ...
      if(state==7){
      	...
      	begin1=true;
        	...
        	if(roomLocation.closed_green_Left()){      	
        		PS_ATTRACTOR.setValue(curr_time,10.0,14.0);
      			abstract_robot.setDisplayString("Entro nella stanza");
        	}
