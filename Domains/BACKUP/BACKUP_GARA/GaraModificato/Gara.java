  ...
  private boolean avanzando;
	...
	public void configure()
		...
		PF_CLOSE_TO_HOMEBASE5 = new b_Close_vv(0.4, PS_GLOBAL_POS,
			...
			// close to homebaseT 
		NodeBoolean
		PF_CLOSE_TO_HOMEBASET = new b_Close_vv(0.4, PS_GLOBAL_POS,
			PS_HOMEBASE5_GLOBAL);
		...
		//======
		v_StaticWeightedSum_va 
		AS_GO_HOMET = new v_StaticWeightedSum_va();

		AS_GO_HOMET.weights[0]  = avoidgain;
		AS_GO_HOMET.embedded[0] = MS_AVOID_OBSTACLES;

		AS_GO_HOMET.weights[1]  = noisegain;
		AS_GO_HOMET.embedded[1] = MS_NOISE_VECTOR;

		AS_GO_HOMET.weights[2]  = mtggain;
		AS_GO_HOMET.embedded[2] = MS_MOVE_TO_HOMEBASE5;
		...
		// STATE 3 GO_TO_HOMEBASET
		STATE_MACHINE.triggers[4][0]  = PF_CLOSE_TO_HOMEBASET;
		...
		STEERING.embedded[5] = AS_GO_HOMET;
		...
		//======
                ...
                SONAR_CONFIGURATION.sonarActived[0] = new int[]{1,3,5,7,9,11,13,15};
                SONAR_CONFIGURATION.sonarPrecision[0] = SonarObjectSensor.LOW;
                SONAR_CONFIGURATION.sonarActived[1] = new int[]{1,4,7,9,15};
                SONAR_CONFIGURATION.sonarPrecision[1] = SonarObjectSensor.MEDIUM;
                SONAR_CONFIGURATION.sonarActived[2] = new int[]{2,4,10,12,14,15};
                SONAR_CONFIGURATION.sonarPrecision[2] = SonarObjectSensor.LOW;
        SONAR_CONFIGURATION.sonarActived[3] = new int[]{0,14,15};
				...
				SONAR_CONFIGURATION.sonarActived[5] = new int[]{};
    ...
    //======
                ...
                LASER_CONFIGURATION.embedded[2] = false;
	...
	public int takeStep() 
            {
				//abstract_robot.setDisplayString(cron.start());	
		...
		//SONAR
                ...
                if(bresult){
					...
					System.out.println("___________________" + "Stato: " + state + "_____________________" );
					System.out.println();
                ...
                if(bresult){
					...
					System.out.println("TIMER: " + leggi());
					System.out.println("________________________________________________");
				}
		...
		//int state=STATE_MACHINE.Value(curr_time);
				...
				else if(state==5)
				{
					if(letto==false)
					{
						System.out.println("TEMPO TOTALE IMPIEGATO: " + leggi());
						letto=true;
					}
					
					abstract_robot.setDisplayString("FINE");
...
/*FERMA E AZZERA IL cronometro. */
  ...
  /*
   * Metodo che fa partire o ripartire il conteggio.
   */
  public void avanza() {
    synchronized (this) {
      avviato_a = System.currentTimeMillis()/1000;
      avanzando = true;
    }
  }
  ...
  public void ferma() {
    synchronized (this) {
      contatore += System.currentTimeMillis()/1000 - avviato_a;
      avanzando = false;
    }
  }

  /** Azzera il cronometro fa partire il conteggio. */
  ...
  */
  ...
  public long leggi() {
    synchronized (this) {
      return avanzando ? contatore + System.currentTimeMillis()/1000 - avviato_a
                       : contatore;
    }
  }
