...
public class Fembot extends ControlSystemMFN150 {
    ...
    public int takeStep() {
        ...
        if(ritardo(state,flag,500)==true)
        {
            return(CSSTAT_OK);
        }
        ...
        if(state==0)
        {
            ...
            if(messaggioRicevuto==true) //invio il messaggio
            {
		...
		if (BufMsg.hasMoreElements()){
 			...
 			if (BufMsg.hasMoreElements()){
                    ...
                    setDelay(0);
                    flag=true;
                    messaggioRicevuto=true; //sta variabile non penso serva
				}      
      			}
   		}
    ...
    public boolean ritardo(int state,boolean flag,int valore_di_ritardo)
    {
         ...
         if(state==0 && flag==true)
        {
            ...
            if(getDelay()<=valore_di_ritardo)
            {
				...
				{
					System.out.println("wait: " +getDelay()/100+" sec");
				aiuto++;
				}
